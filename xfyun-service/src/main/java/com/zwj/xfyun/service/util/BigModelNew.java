    ...
    public static void main(String[] args) throws Exception {
      ...
      while (true){
          if(totalFlag){
              ...
              // 构建鉴权url
              String authUrl = getAuthUrl(XfyunConstants.HOST_URL, XfyunConstants.API_KEY, XfyunConstants.API_SECRET);
              OkHttpClient client = new OkHttpClient.Builder().build();
              String url = authUrl.toString().replace("http://", "ws://").replace("https://", "wss://");
              Request request = new Request.Builder().url(url).build();
              for (int i = 0; i < 1; i++) {
                  totalAnswer="";
                  WebSocket webSocket = client.newWebSocket(request, new BigModelNew("", false));
              }
          }else{
              Thread.sleep(200);
          }
    ...
    class MyThread extends Thread {
        ...
        public void run() {
            try {
                ...
                header.put("app_id",XfyunConstants.APP_ID);
